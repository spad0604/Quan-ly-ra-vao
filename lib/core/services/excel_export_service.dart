...
class ExcelExportService {
  ...
  Future<bool> openFile(String filePath) async {
    try {
      ...
      if (Platform.isWindows) {
        // Use the OS default associated application. Use unquoted arg so cmd handles spacing.
  ...
  Future<bool> revealInFolder(String filePath) async {
    try {
      ...
      if (Platform.isWindows) {
        // explorer expects a single arg like "/select,C:\\path\\to\\file"
      ...
      print('Error revealing file in folder: $e');
      return false;
    }
  }
    ...
    try {
      ...
      for (int i = 0; i < members.length; i++) {
        ...
        sheet
            ...
            .value = TextCellValue(
          member.officerNumber.isEmpty ? '' : member.officerNumber,
        );
        sheet
        ...
        sheet
            ...
            .value = TextCellValue(
          member.position.isEmpty ? '' : member.position,
        );
        sheet
  ...
  Future<String?> exportHistory(
    ...
    try {
      ...
      final headers = isStaff
          ...
          : ['STT', 'Họ và tên', 'Số CCCD', 'Thời gian vào', 'Lý do'];
      ...
      for (int i = 0; i < historyList.length; i++) {
        ...
        if (isStaff) {
          ...
          final checkOutTime = item['check_out_time'] != null
              ? DateFormat('dd/MM/yyyy HH:mm:ss').format(
                  DateTime.fromMillisecondsSinceEpoch(
                    item['check_out_time'] * 1000,
                  ),
                )
              : '';
          final status = item['check_out_time'] != null
              ? 'Đã ra'
              : 'Đang trong';
      ...
      final filePath =
          '${directory.path}/Lich_su_ra_vao_${fileTypeStr}_$timestamp.xlsx';
      final file = File(filePath);
      await file.writeAsBytes(excel.encode()!);
